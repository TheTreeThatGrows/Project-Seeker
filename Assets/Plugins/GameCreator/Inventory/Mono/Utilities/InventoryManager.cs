namespace GameCreator.Inventory
{
	...
	public class InventoryManager : Singleton<InventoryManager>, IGameSave
	{
        ...
        private void OnChangeProfile(int prevProfile, int nextProfile)
        {
		...
		public int AddItemToInventory(int uuid, int amount = 1)
		{
		...
		public int SubstractItemFromInventory(int uuid, int amount = 1)
		{
			if (!this.itemsCatalogue.ContainsKey(uuid))
			{
				Debug.LogError("Could not find item UUID in item catalogue");
		...
		public bool BuyItem(int uuid, int amount, Merchant merchant = null)
		{
			if (!this.itemsCatalogue.ContainsKey(uuid))
			{
				Debug.LogError("Could not find item UUID in item catalogue");
		...
		public bool SellItem(int uuid, int amount, Merchant merchant = null)
		{
			if (!this.itemsCatalogue.ContainsKey(uuid))
			{
				Debug.LogError("Could not find item UUID in item catalogue");
        ...
        public bool CanEquipItem(GameObject target, int uuid, int itemType)
        {
        ...
        public int HasEquiped(GameObject target, int uuid)
        {
            CharacterEquipment equipment = this.RequireEquipment(target);
            return (equipment != null ? equipment.HasEquip(uuid) : 0);
        }

        public bool HasEquipedTypes(GameObject target, int itemTypes)
        {
            CharacterEquipment equipment = this.RequireEquipment(target);
            return equipment != null && equipment.HasEquipTypes(itemTypes);
        }

        public Item GetEquip(GameObject target, int itemType)
        {
            CharacterEquipment equipment = this.RequireEquipment(target);
        ...
        public bool Unequip(GameObject target, int uuid)
        {
            CharacterEquipment equipment = this.RequireEquipment(target);
        ...
        public bool UnequipTypes(GameObject target, int itemTypes)
        {
            CharacterEquipment equipment = this.RequireEquipment(target);
        ...
        private CharacterEquipment RequireEquipment(GameObject target)
        {
